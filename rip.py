    ...
    for entry in riplist:
        ...
        if TK in RTable:	# we have an existing entry for this destination
            ...
            if (
                ...
                call_list = ['/sbin/ip', 'route', 'change', ipaddr + '/' + netmask, 'via', update_sender]
                print ('updating route to {}/{}'.format(ipaddr, slash(aton(netmask))), ' advertised by router r{}'.format(update_sender[-1])) 
                if MODTABLES: subprocess.call(call_list)
                #print('/sbin/ip', 'route', 'change', '-net', ipaddr, 'netmask', netmask, 'gw', update_sender)
           ...
           print ('adding route to new destination {}/{}'.format(ipaddr, slash(aton(netmask))), ' advertised by router r{}'.format(update_sender[-1]))
           if MODTABLES: subprocess.call(call_list)
